...
| `enums` | 跨服务共享的机构与会话枚举 | `OrganType`、`SessionType`（含 `MEMBER`） |
| `web` | Principal、请求头、JWT/DPoP 数据和 ScopedValue 上下文 | `PrincipalContext`、`PrincipalHeaders`（含 `MEMBER_ID`）、`TokenJWTPayload`、`ScopedContextHolder` |
